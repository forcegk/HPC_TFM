...
\subsection{Implementación y ventajas de localidad}
\label{ssec:implementacion_ventajas_localidad}
...
Para una multiplicación de matrices similar a la tratada en esta misma sección ($w^{d} \times x^{D} = out^{D}$, de dimensiones $m \times n$, $n \times k$ y $m \times k$), representando la dimensión común de medida $n$ en el eje horizontal, se puede apreciar la siguiente multiplicación de matrices, con el correspondiente \textit{layout} en memoria: 
\begin{gather}
    \begin{pmatrix}
        w_{00} & w_{01}\\
        0 & w_{11}\\
