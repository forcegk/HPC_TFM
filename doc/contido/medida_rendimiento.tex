...
En este caso solamente se emplean librerías estándar, por lo que para GCC, en las líneas de compilación simplemente se cambia \texttt{-s} por \texttt{-g3}. Por otro lado y para emplear como contraste, también se emplean los dos compiladores de Intel, ICC, y el nuevo ICX\footnote{Documentación empleada para compilación, así como para posibles líneas de optimización futura, están disponibles en \url{https://www.intel.com/content/www/us/en/developer/articles/guide/porting-guide-for-icc-users-to-dpcpp-or-icx.html}.}:\medskip
\begin{lstlisting}[language=bash]
...
Así, se puede apreciar que la mejoría del speedup con un dataset 10 veces más grande es cada vez más alta a partir del 90\% de \textit{sparsity} para la implementación punto a punto, siendo sin embargo inferior el rendimiento basado en librsb.
...
Por otro lado, si se leen proporcionalmente los mismos valores en coma flotante de memoria principal, pero se tarda cinco veces más, esto repercute en un descenso a la quinta parte en FLOPS (es decir, un descenso a la quinta parte en el eje $y$).
...
%%%% TODO MODIFICAR ROOFLINEN SPARSE ESTIMADO %%%%
...
Estos decepcionantes resultados, sin embargo, motivan la creación de arquitecturas punto a punto, ya que implica que existe un amplio margen de mejora mediante análisis estático de código y generación de instrucciones FMA y de control \textit{ad-hoc}.


\subsection{Código \textit{point-to-point}}
El código punto a punto es mucho más demandante a nivel de caché de instrucciones, así como en análisis estático de código, con tiempos de compilación que se acercan a las 24 horas, y decenas de gigabytes de memoria RAM necesaria. Esta aproximación, sin embargo, debido a lo que se presenta a continuación, se espera que tenga un amplio margen de mejora, siendo posible incluso vectorizar y reordenar gran parte de las instrucciones mediante el empleo de la herramienta MACVETH\footnote{\url{https://github.com/UDC-GAC/MACVETH}}.

%% TODO Inserstar múltiples rooflines